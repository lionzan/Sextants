...
class MPU9250_
{
    const uint8_t MPU9250_ADDRESS {0x69};  // Device address when ADO = 0 0x68 | ADO=1 0x69
    const uint8_t AK8963_ADDRESS {0x0C};   //  Address of magnetometer

    const uint8_t MPU9250_WHOAMI_DEFAULT_VALUE {0x73}; // 0x68???? MODIFIED TO MATCH MPU BOARD
...
public:
    ...
    float getMagCal(uint8_t i) const { return (i < 3) ? magCalibration[i] : 0.f; }
    ...
    void print(String what) const
    {
        if (what.indexOf("a")!=-1) {printRawAcc();}
        if (what.indexOf("g")!=-1) {printRawGyro();}
        if (what.indexOf("m")!=-1) {printRawMag();}
        if ((what.indexOf("r")!=-1) || (what.indexOf("p")!=-1) || (what.indexOf("y")!=-1)) {printRollPitchYaw();}
        if (what.indexOf("c")!=-1) {printCalibration();}

    }

    void printRawAcc() const
    {
    ...
    void printRawMag() const
    {
        // Print mag values in degree/sec
        Serial.print(m[0]);
        Serial.print(", "); Serial.print(m[1]);
        Serial.print(", "); Serial.println(m[2]);
    }

    void printRawQuat() const
    {
        // print quaternion
